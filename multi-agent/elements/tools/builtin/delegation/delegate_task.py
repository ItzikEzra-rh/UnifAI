...
from elements.nodes.common.workload import Task, WorkPlanService, WorkItemStatus
        ...
        try:
            ...
            if args.parent_item_id:
                ...
                try:
                    ...
                    # Load plan, mark item as delegated, and save
                    plan = service.load(thread_id, owner_uid)
                    if plan and args.parent_item_id in plan.items:
                        item = plan.items[args.parent_item_id]
                        item.status = WorkItemStatus.WAITING
                        item.correlation_task_id = task.task_id
