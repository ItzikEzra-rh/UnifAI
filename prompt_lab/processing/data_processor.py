...
class DataProcessor:
    ...
    def process_element(self, element_data, index):
        ...
        for group_name, options in input_option_groups.items():
            for category, templates in options.items():
                ...
                if self.tokenizer.is_within_limit(formatted_input):
                    ...
                    print(f"*********************** Processing Element {index + 1}: {element_type} - {group_name}/{category} ***********************")
                    print(f"\n{output}\n")

        self.output_repository.save_progress(index + 1)  # Save progress after each element
    ...
    def process_all_elements(self):
        ...
        for index, element in enumerate(data[self.current_index:], start=self.current_index):
            self.process_element(element, index)
        self.save_data()
