          ...
          <div className="flex-1 min-h-0">
            <Tabs defaultValue="nodes" className="h-full flex flex-col">
              <TabsList className="grid w-full grid-cols-3 bg-gray-800">
                <TabsTrigger value="nodes" className="text-gray-300 data-[state=active]:text-white text-xs">
                  Nodes ({buildingBlocks.length})
                </TabsTrigger>

                <TabsTrigger value="orchestrators" className="text-gray-300 data-[state=active]:text-white text-xs">
                  Orchestrators ({orchestrators.length})
                </TabsTrigger>

                <UmamiTrack event={UmamiEvents.AGENT_GRAPHS_CONDITIONS_BUTTON} includeUserData={false}>
                  <TabsTrigger value="conditions" className="text-gray-300 data-[state=active]:text-white text-xs">
                    ...
                    {orchestrators.map((block) => {
                      ...
                      return (
                        <Card
                          ...
                          className={`transition-colors ${
                            isUsed
                              ? 'bg-cyan-950 border-cyan-800 opacity-50 cursor-not-allowed'
                              : 'bg-cyan-900/40 border-cyan-700 hover:border-cyan-600 cursor-grab active:cursor-grabbing'
                          }`}
                          draggable={!isUsed}
                          onDragStart={(event) => handleDragStart(event, block)}
                        >
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2 flex-1">
                              <div className="flex items-center justify-center w-8 h-8 rounded-full text-xs font-semibold text-white"
                                ...
                                {getCategoryDisplay(block.workspaceData?.category || "orchestrators").icon}
                              </div>
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                  <h4 className={`font-medium text-sm truncate ${isUsed ? 'text-gray-500' : 'text-white'}`}>
                                  ...
                                  {isUsed && (
                                    <span className="text-xs bg-cyan-800 text-cyan-300 px-1.5 py-0.5 rounded">
                                      Used
                                    </span>
                                  )}
                                </div>
