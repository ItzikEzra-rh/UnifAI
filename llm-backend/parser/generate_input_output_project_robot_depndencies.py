...
dependencies_prompt = """Analyze the following Robot Framework test code to explain its dependencies, relationships, and file paths, with a clear and structured format.

Test code: "{code}"

**Imports and Locations:**  
Imports File Locations: "{imports_file_locations}"  
Test File Location: "{file_location}"  

The explanation should cover:

1. **Detailed List of External Resources**: 
   - For each imported resource, specify its role in the test.
   - Explain why each resource is necessary for the test to function.
   - Include any custom keywords or functions that these resources provide, and describe how they support the test cases.

2. **Path Breakdown**: 
   - Describe both the relative and absolute paths used in the test code.
   - For each relative path listed in the imports, provide the corresponding absolute path.
   - Clarify how these paths connect within the overall file structure of the project, indicating their locations and relationships.

3. **Test File Setup and Dependencies**: 
   - Explain how the test suite’s setup and teardown procedures work with the imported resources.
   - Describe the specific role of suite setup and teardown functions (e.g., `Rbac Suite Setup`, `Rbac Suite Teardown`) in preparing and cleaning up the test environment.
   - Highlight any dependencies between the setup/teardown steps and the external resources.

4. **Test Case Structure**: 
   - Provide a brief overview of each test case, identifying any dependencies on external resources.
   - Explain how imported keywords are applied within the test cases.
   - Illustrate how these keywords and resources connect to specific parts of the test code, indicating their importance to the test structure.

**Requirement:** 
- The explanation should be structured, with numbered sections for each of the points above.
- Include relevant code snippets where needed to clarify dependencies and relationships.
- Emphasize the relationship between the relative and absolute paths, ensuring clarity in how the files are connected within the project structure.
"""
...
def process_element(element_data):
    ...
    if not is_more_than_ctx(prompt):
        ...
        dataset.append(
            {"input": input_text, "output": output, "element_type": element_type, 'input_type': 'dependencies_prompt'})
...
def process_json(json_data, output_file, progress_file):
    ...
    file_elements = [elem for l in json_data for elem in l if
                     elem.get("type") == "Test" or elem.get("type") == "Resource"]
    total_elements = len(file_elements)

    # Process Classes and their Methods
    for idx, file_data in enumerate(file_elements, start=1):
        print(f"************************** {idx}/{total_elements} ***************************************")
        if idx < current_index:  # Skip elements already processed
            continue
        # Process the file and its elements itself
        full_dataset.extend(process_element(file_data))
...
# File paths
json_file_path = r"/home/instruct/AI-TC-s-Generator/llm-backend/parser/RHOAI_Files_Mapping.json"
output_file = r"/home/instruct/AI-TC-s-Generator/llm-backend/parser/RHOI_files_dep_processed_dataset.json"
