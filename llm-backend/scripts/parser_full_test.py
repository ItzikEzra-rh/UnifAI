...
def create_full_test(settings, variables, test_cases_dict, keywords):
    keywords_dict = parse_keywords(keywords)
    test = "*** Settings ***\n\n"
    test += f"{settings}\n\n"
    test += "*** Variables ***\n\n"
    test += f"{variables}\n\n"
    test += "*** Test Cases ***\n\n"
    for test_case, test_data in test_cases_dict.items():
        test += f'{test_case}\n'
        test += f"{test_data['code']}\n\n"
    test += "*** Keywords ***\n\n"
    for keyword_code in keywords_dict.values():
        test += f"{keyword_code}\n\n"
    return test
...
for i, file_path in enumerate(robot_files):
    ...
    try:
        with open(file_path, 'r') as file:
            ...
            if test_cases_dict:
                documentation = ""
                for test_val in test_cases_dict.values():
                    documentation += f"{test_val['documentation']}\n"
                data = create_full_test(settings, variables, test_cases_dict, keywords)
                tc = {
...
print(len(dataset))
with open('ncs_412_full_tests.json', 'w') as f:
