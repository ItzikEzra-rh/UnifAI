...
class OadpMetaDataExtractor(MetaDataExtractorBase):
    TOP_100_TERMS = ['backup', 'velero', 'namespace', 'brcase', 'application', 'error', 'backupspec', 'backuprestorecase', 'client', 'restore', 'dpaclient', 'dpa', 'time', 'spec',
                     'case', 'code', 'backuprestoreappcase', 'package', 'pod', 'mysql', 'resource', 'type', 'includednamespaces', 'app', 'kopia', 'ginkgo', 'customresource', 'configuration',
                     'genericapp', 'file', 'phase', 'status', 'schedule', 'parameter', 'functionality', 'restic', 'backuprestorename', 'command', 'vsl', 'bucket', 'item', 'secret', 'structure',
                     'csi', 'storage', 'label', 'credential', 'step', 'oadpnamespace', 'pointer', 'restorespec', 'background', 'config', 'policy', 'preparebackup', 'image', 'setupdpa', 'django',
                     'class', 'backupname', 'clientset', 'bsl', 'backupnativedatamover', 'provider', 'custom', 'pvc', 'role', 'volume', 'method', 'backupcsi', 'output', 'cluster', 'node', 'message',
                     'datum', 'flag', 'storageclass', 'oadp', 'hook', 'value', 'podconfig', 'wait', 'setclient', 'deployment', 'podlist', 'entry']
    ...
    def is_pure_action_word(word: str) -> bool:
        ...
        """
        if OadpMetaDataExtractor.is_technical_term(word):
            return False
