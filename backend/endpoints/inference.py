...
from providers.inference import add_inference_counter_per_each_model, \
                              retrieve_inference_counter, retrieve_inference_counter_all

inference_bp = Blueprint("inference", __name__)

@inference_bp.route("/addInferenceCounter", methods=["POST"])
@from_body({
...
@inference_bp.route("/inferenceCounter", methods=["GET"])
@from_query({
    "model_id":        fields.Str(required=True, data_key="modelId"),
})
...
@inference_bp.route("/inferenceCounterAll", methods=["GET"])
