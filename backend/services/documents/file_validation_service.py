...
Validation Flow:
1. UI selects files → UI calls /docs/validate with file metadata
2. Backend returns validation results (valid files + errors for invalid ones)
3. UI uploads only the valid files
4. UI calls /pipelines/embed with skip_validation=true (since pre-validated)
5. External API calls /pipelines/embed with skip_validation=false (default) for full validation

Note: MD5 duplicate checking is NOT performed here because it requires the actual file content.
MD5 validation happens during registration (after file upload) and only blocks duplicates
...
class FileValidationService:
    ...
    Usage:
        ...
        # result.valid_files contains files that passed all validations
        # result.errors contains details about validation failures
    """
    ...
    def validate(
        ...
        Example usage:
            ...
            for valid_file in result.valid_files:
